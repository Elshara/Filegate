...
function fg_render_setup_page(array $data = []): void
{
    ...
    foreach ($configurations as $asset => $configuration) {
        ...
        $body .= '<article class="asset-card" data-asset="' . htmlspecialchars($asset) . '">';
        ...
        foreach ($roles as $role => $roleDescription) {
            ...
            $body .= '<label><input type="checkbox" name="allowed_roles[]" value="' . htmlspecialchars($role) . '"' . $checked . '> ' . htmlspecialchars(ucfirst($role)) . '</label>';
        }
        $body .= '</div>';
        $checkedOverride = $allowUserOverride ? ' checked' : '';
        $body .= '<label class="field-toggle"><input type="checkbox" name="allow_user_override" value="1"' . $checkedOverride . '> Allow members to personalise this asset</label>';
        $body .= '</fieldset>';
